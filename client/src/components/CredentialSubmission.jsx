                    ...
                    <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll '>
                        {credential.map((cred, index) => (
                            <div key={cred.type} className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2'>
                                ...
                                <input type="text" value={cred.value} onChange={(e) => setCredential((prev) => prev.map((c, i) => (i === index ? { ...c, value: e.target.value } : c)))} className='w-full text-sm border border-gray-300 rounded outline-indigo-400' />
                                <X className='w-5 h-5 text-gray-500 hover:text-gray-700' onClick={() => setCredential((prev) => prev.filter((_, i) => i !== index))} />
                            </div>
                        ))}
