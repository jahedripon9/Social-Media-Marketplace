        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
          ...
          <Section title='Account Metrics'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
              ...
              <InputField label='Followers Count *' type='number' value={formData.followers_count} placeholder='10000' onChange={(v) => handleInputChange('followers_count', v)} required={true} min='0' />
              ...
              <InputField label='Monthly Views/Impressions' type='number' value={formData.monthly_views} placeholder='100000' onChange={(v) => handleInputChange('monthly_views', v)} min={0} />
            </div>
          ...
          <Section title='Screenshots & Proof'>
            <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
              ...
              <p className='text-sm text-gray-500 mt-2'>Upload Screenshots or Proof of account analysis</p>
            </div>
          ...
          <div className='flex justify-end gap-3 text-sm'>
            <button type='button' className='px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors' onClick={() => navigate(-1)}>
              Cancel
            </button>
...
const TextareaField = ({ label, value, onChange, required = false }) => (
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <textarea rows={5} required={required} value={value} onChange={(e) => onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-indigo-500 border-gray-300 focus:ring-2' />
  </div>
)
